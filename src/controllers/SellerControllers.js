...
const imgProductSellerUpdate = updateProductImg.single(
    "imgProductSellerUpdate"
);
                    ...
                    <tbody>
                        ...
                        <tr>
                            <td style="padding:0;padding-top:25px;font-family:'Segoe UI',Tahoma,Verdana,Arial,sans-serif;font-size:14px;color:#2a2a2a">
                                Gunakan kode keamanan berikut untuk akun <span class="il">Aslilokal Seller</span> <a dir="ltr" id="m_6439999066462717123iAccount" class="m_6439999066462717123link" style="color:#2672ec;text-decoration:none" href="` +
                        newSeller.emailSeller +
                        `" target="_blank">` +
                        newSeller.emailSeller +
                        `</a>.
                            </td>
                        ...
                        <tr>
                            <td style="padding:0;padding-top:25px;font-family:'Segoe UI',Tahoma,Verdana,Arial,sans-serif;font-size:14px;color:#2a2a2a">
                                Kode keamanan: <span style="font-family:'Segoe UI Bold','Segoe UI Semibold','Segoe UI','Helvetica Neue Medium',Arial,sans-serif;font-size:14px;font-weight:bold;color:#2a2a2a">` +
                        tokenVerify +
                        `</span>
                            </td>
                        </tr>
                ...
                mg.messages().send(data, async (error, body) => {
                    if (error) {
                        ...
                        return res.status(200).json({
                            success: false,
                            message:
                                "Maaf ada gangguan email service! Silahkan daftar ulang!",
                        });
                    } else {
                        try {
                            ...
                            return res.status(200).json({
                                success: false,
                                message:
                                    "Maaf ada gangguan database service! Silahkan daftar ulang!",
                            });
                        }
                    }
                });
        ...
        await SellerShop.findOneAndUpdate(
            ...
            },
            {
                $set: {
                    ...
                    isShopFreeDelivery: req.body.isShopFreeDelivery,
...
export const putOneDebtorLunas = async (req, res) => {
    try {
        const { nameDebtor, totalDebt, descDebt, statusTransaction } = req.body;
...
export const getOneNotificationSeller = async (req, res) => {
    try {
        const oneNotification = await Notification.findByIdAndUpdate(
